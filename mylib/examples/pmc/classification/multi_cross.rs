...
fn main() {
    ...
    let x_test = x.clone();
    let y_test = y.clone();
    let x_test_flatten: Vec<f32> = x_test.clone().into_iter().flatten().collect::<Vec<f32>>();
    let x_ptr_test : *mut f32 = Vec::leak(x_test_flatten.clone()).as_mut_ptr();
    let y_ptr_test : *mut f32 = Vec::leak(y_test.clone()).as_mut_ptr();
    ...
    train_mlp(mlp, x_ptr, y_ptr, data_size as u32, x_ptr_test, y_ptr_test, data_size as u32, 0.001, 1000000);
