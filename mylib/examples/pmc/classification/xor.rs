...
fn main() {
    ...
    let npl: Vec<u32> = vec![2, 3, 1];
    let mlp: *mut MultiLayerPerceptron = init_mlp(npl.as_ptr(), npl.len() as u32, true);


    let c_log_filename = CString::new("clf_xor").expect("CString::new failed");
    let c_model_filename = CString::new("../models/examples/mlp/classification/xor.json").expect("CString::new failed");
