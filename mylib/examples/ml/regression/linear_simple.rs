...
fn main() {
    ...
    let linear_model: *mut LinearModel = init_linear_model(2);
    train_linear_model(linear_model, x_ptr, y_ptr, data_size as u32, 0.01, 1000_000, false);
    
    println!("");
    println!("MODEL LINEAR: Regression Linear_simple");
    for i in 0..data_size {
        let input_ptr: *mut f32 = Vec::leak(x[i].clone()).as_mut_ptr();
        let output = predict_linear_model(linear_model, input_ptr, false);
