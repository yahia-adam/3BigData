...
fn get_weights(model: *const LinearRegression) -> Vec<f32>
{
...
fn set_weights(model: *mut LinearRegression, weights: Vec<f32>) {
...
fn guess(model: *const LinearRegression, inputs: Vec<f32>) -> f32
{
...
#[no_mangle]
pub extern "C" fn to_string(model: *const LinearRegression) -> *const c_char {
    ...
    let c_str: CString = CString::new(res).expect("Failed to convert string to CString");
